...
function changeDOM() {
  ...
  drawHistogramOnTable(
    tableWidgetApiCall.widgetDataContainer.dataFrame.cols.length,
    tableWidgetApiCall.widgetDataContainer.dataFrame.rows.length,
  );
  $('div [id="' + tableWidgetGUID + '"]')
    .find('div [id^=tableHistogramDiv]')
    .animate(
      {
        opacity: 1 / 4,
      },
...
function drawHistogramOnTable(tableColumnsQuantity, tableRowsQuantity) {
  ...
  $('div [id="' + tableWidgetGUID + '"]')
    .find('div [id^=tableHistogramDiv]')
    .remove();
  let lengthAllTableCells = $('div [id="' + tableWidgetGUID + '"]')
    .find('td')
    .not('.dx-last-cell')
    .not('.dx-pivotgrid-expanded').length;
  ...
  for (let columnIterator = 0; columnIterator < tableColumnsToDraw.length; columnIterator++) {
    ...
    let curColumnMaxValue = getcurColumnMaxValue(
      curColumnNumber,
      tableColumnsQuantity,
      lengthAllTableCells,
    );
    for (
      let cellIterator = startDrawingFromCell + tableColumnsToDraw[columnIterator];
      cellIterator < lengthAllTableCells;
      cellIterator += tableColumnsQuantity
    ) {
      let curCell = $('div [id="' + tableWidgetGUID + '"]')
        .find('td')
        .not('.dx-last-cell')
        .not('.dx-pivotgrid-expanded')[cellIterator];
      ...
      if (curCellValue > 0 && Object.is(curCellValue, NaN) === false) {
        ...
        let curCellHistogramWidth = getHistogramWidth(
          curCellValue,
          curColumnMaxValue,
          curCellWidthValue,
          maxHistogramWidth,
          minimalHistogramWidth,
        );
  ...
  function getcurColumnMaxValue(thisColumnNumber, tableColumnsQuantity, lengthAllTableCells) {
    ...
    for (
      let cellIterator = startDrawingFromCell + thisColumnNumber;
      cellIterator < lengthAllTableCells;
      cellIterator += tableColumnsQuantity
    ) {
      thisItemCell = $('div [id="' + tableWidgetGUID + '"]')
        .find('td')
        .not('.dx-last-cell')
        .not('.dx-pivotgrid-expanded')[cellIterator];
      thisValue = parseFloat(
        $(thisItemCell).children('span')[0].textContent.trim().replace(/ /g, ''),
      );
  ...
  function getHistogramWidth(
    thisCellValue,
    thisColumnMaxValue,
    thisColumnWidthValue,
    maxHistogramWidth,
    minimalHistogramWidth,
  ) {
    let thisValueRatio = parseFloat(thisCellValue / (thisColumnMaxValue / 100));
    let thisCellHistogramWidth =
      thisValueRatio *
      0.01 *
      ((parseFloat(thisColumnWidthValue) - 18) * (maxHistogramWidth * 0.01));
