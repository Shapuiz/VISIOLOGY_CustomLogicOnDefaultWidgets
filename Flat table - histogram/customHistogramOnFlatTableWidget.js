...
const maximumValuesTextWidgetGUIDs = [
  'b8459e0632084431b67e01c89d4184a9',
  '5f67ad64902c49ebb305c94cfae99de1',
]; //Array of "Text" widget GUIDs with maximum column value
...
function drawHistogramOnTable(tableColumnsQuantity, tableRowsQuantity) {
  ...
  for (let columnIterator = 0; columnIterator < tableColumnsToDraw.length; columnIterator++) {
    let curColumnMaxValue = parseFloat(
      $('div[id="' + maximumValuesTextWidgetGUIDs[columnIterator] + '"]').find('div')[0]
        .textContent,
    );
    for (
      ...
      let curCellValue = parseFloat(
        $('div [id="' + tableWidgetGUID + '"]')
          .find('td')
          [cellIterator].textContent.trim()
          .replace(/ /g, ''),
      );
